...
RouterCreateScan(CustomScan *scan)
{
	...
	else
	{
		...
		if (IsMultiRowInsert(workerJob->jobQuery) ||
			((multiPlan->operation == CMD_UPDATE || multiPlan->operation == CMD_DELETE) &&
			 MultiShardConnectionType == SEQUENTIAL_CONNECTION))
		{
			/*
			 * Multi-row INSERT and multi shard update detele are executed
			 * sequentially instead of using parallel connections.
			 */
			scanState->customScanState.methods = &RouterSequentialModifyCustomExecMethods;
		}
		else
